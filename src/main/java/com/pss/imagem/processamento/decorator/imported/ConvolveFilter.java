...
package com.pss.imagem.processamento.decorator.imported;
	...
	public static void convolveHV(Kernel kernel, int[] inPixels, int[] outPixels, int width, int height, boolean alpha, int edgeAction) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				int ia = alpha ? MyPixelUtils.clamp((int)(a+0.5)) : 0xff;
				int ir = MyPixelUtils.clamp((int)(r+0.5));
				int ig = MyPixelUtils.clamp((int)(g+0.5));
				int ib = MyPixelUtils.clamp((int)(b+0.5));
